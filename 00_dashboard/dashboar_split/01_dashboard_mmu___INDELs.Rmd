...
snpeff_data_indel$cts_by_geno_region %>% arrange(desc(Percent)) %>% arrange(desc(Percent)) %>% kable(caption = "cts_by_geno_region") %>% kable_styling(full_width = F) 

snpeff_data_indel$vars_by_type %>% arrange(desc(Percent)) %>% kable(caption = "vars_by_type") %>% kable_styling(full_width = F) 

snpeff_data_indel$eff_by_impact %>% arrange(desc(Percent)) %>% kable(caption = "eff_by_impact") %>% kable_styling(full_width = F) 
```
...
Part2-INDEL-RDDs {data-navmenu="INDELs"}
=====================================

### Intro

* Find regions of distinct genetic differentiaton for each line, relative to FZTDU.

### ...

```{r indels_win_fst_data_sel_vs_ctrl_zscore, include=F}  

win_fst_indel_sel_vs_ctrl <- lapply(
  
  list.files(here("batches123_07_selection_analysis/03_fst_indels/output"),
             pattern = "windowed.weir.fst$"), 
  
  function(fl){
    
    read.table(
      here("batches123_07_selection_analysis/03_fst_indels/output",fl),
      header = T,
      stringsAsFactors = F
      )
    }
...
names(win_fst_indel_sel_vs_ctrl) <- list.files(
  here("batches123_07_selection_analysis/03_fst_indels/output"),
  pattern = "windowed.weir.fst$"
  ) %>% str_remove(".windowed.weir.fst")

# prepare data (i.e. zscore transformation) for visualizations
win_fst_indel_sel_vs_ctrl_prep <- lapply(win_fst_indel_sel_vs_ctrl, function(d){
  d %>% 
    # filter by min number of SNPs in window (min 10 SNPs)
    dplyr::filter(N_VARIANTS >= 5) %>% 
...
win_fst_indel_sel_vs_ctrl_prep %>% group_by(contrast) %>% summarise(n())
```

```{r indel_summary_win_zfst}
summary_z_win_fst_indel <- win_fst_indel_sel_vs_ctrl_prep %>% 
  ungroup() %>% 
  group_by(contrast) %>% 
  summarise(
    n_win = n(),
...
summary_z_win_fst_indel %>% kable(digits = 2, caption = "z(Mean Window Fst)") %>% kable_styling(full_width = F)

q_zfst_indels <- summary_z_win_fst_indel %>% 
...
```{r indels_define_function_to_find_distinct_REDs} 

find_distinct_REDs_indels <- function(contr, rest, q_low, q_high){

  # for each contrast in rest, find bottom 
  rest_bottom_windows <- lapply(rest, function(x){
    
    # set threshold for bottom scores
    q_low_threshold <- summary_z_win_fst_indel[summary_z_win_fst_indel$contrast == x, q_low] %>% as.numeric()
    
    # get rest-contrast individual data
    win_fst_indel_sel_vs_ctrl_prep %>% filter(contrast %in% x) %>% 
  ...
  # set upper score
  q_high_threshold <- summary_z_win_fst_indel[summary_z_win_fst_indel$contrast == contr, q_high] %>% as.numeric()
  
  # get top windows in contrast of interest
  contr_extreme_windows <- win_fst_indel_sel_vs_ctrl_prep %>% 
    filter(
  ...
  distinct_windows <- inner_join(contr_extreme_windows, 
    ...
    inner_join(
      win_fst_indel_sel_vs_ctrl_prep %>% 
...
```{r indels_export_gene_lists, eval = FALSE}

lapply(1:length(RDD_indels_genes), function(i){
  
  pop <- names(RDD_indels_genes)[i]
  out_nm <- paste0("RDD_INDELs_genes_",pop,".csv")
  RDD_indels_genes[[i]] %>% as.data.frame() %>% dplyr::select(mcols.gene_id) %>% 
    write.table(here("00_dashboard/data/RDD_genes_INDELs",out_nm), quote = FALSE, row.names = FALSE, col.names = FALSE)
...
Part3-INDELs_in_SNP-RDD-Genes {data-navmenu="INDELs"}
===============================================
...
* Find INDELs in genes overlapping regions of distinct genetic differentiation identified based on SNPs
...
### Vis_Fst_Indels
```{r heatmap_fst_high_moderte_indels, fig.height=10, fig.width=10, eval = FALSE} 
