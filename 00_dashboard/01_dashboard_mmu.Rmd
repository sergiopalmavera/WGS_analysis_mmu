...
library(Gviz)
=======
>>>>>>> 99fb242906a0ad4985739101e2c231ac8c460f08
```
...
```{r for_dashboard_and_manuscript_figure4B, fig.width=7,fig.height=7}
...
display_snp_effects_per_gene <- function(pop){
  ...
  snp_effect_fst <- 
    ...
    # turn data into granges
    makeGRangesFromDataFrame(keep.extra.columns = TRUE)

  # find overlaps between RDD-genes and annotated SNPs
  ov <- findOverlaps(query = x, subject = snp_effect_fst, minoverlap = 1)
  
  # prepare table
  tab <- snp_effect_fst[subjectHits(ov)] %>% 
    as.data.frame() %>% 
  ...
  write.csv(tab, here("00_dashboard/data/RDD_genes", csv_nm))
}
...
- Looking at the functional annotation of RDD-genes (GOBP) and results of ORA analysis, the following genes could be interesting:
  ...
  - **DUK**: 
    ...
    - ORA showed as significantly enriched the term **"Phospholipase D signaling pathway"** with the RDD-genes (Raf1, Adcy6, Grm8, Tsc1, Ralgds). 
      ...
      - The gene **Tsc1** appears to be involved in reproduction and **contains 3 moderate-missense mutations with Fst>0.95.**
  ...
  - **DuhLB**: 
    ...
    - Aldh3a2
      ...
      - **3 moderate-missense variants with Fst~0.7**
    - Pfkfb3
  ...
  - **DU6**
    ...
    - However, these genes could be related to body size:
      ...
      - Atp11b  
        ...
        - **missense_variant with complete differentiation (Fst = 1)**
  ...
  - **DU6P**
    ...
    - Afap1l2
      ...
      - **missense_variant with high differentiation Fst~0.95 **
...
#ens_gene_id = "ENSMUSG00000031870"
#pop="DUC"
#bp_expand=5000


vis_goi_macro <- function(ens_gene_id, pop, bp_expand){
  
  
  # merge (full join) fst windows
  win_fst_merged_gr <- lapply(1:length(win_fst_sel_vs_ctrl), 
         function(i){
         x <- win_fst_sel_vs_ctrl[[i]] %>% 
            dplyr::select(CHROM, BIN_START, BIN_END, MEAN_FST)
         
         names(x)[names(x) == "MEAN_FST"] <- names(win_fst_sel_vs_ctrl)[i]
         return(x)
         }) %>% 
  ...
  goi_gr <- RDD_genes[[pop]][RDD_genes[[pop]]$mcols.gene_id == ens_gene_id]
  
  # expand location 50K to each side
  goi_gr_expanded <- resize(goi_gr, width = width(goi_gr)+(bp_expand*2), fix = "center")
  ...
  # get genes in window including GOI
  gene_track <- GeneRegionTrack(TxDb.Mmusculus.UCSC.mm10.ensGene, 
                                names = "Gene Model",
                                chromosome=unique(seqnames(wins)), 
                                start=start(wins)[1], 
                                end=end(wins)[length(wins)],
                                geneSymbol = TRUE, showId=TRUE)
  
  # since some genes extend too much, extend window too 
  wins2 <- subsetByOverlaps(win_fst_merged_gr,
                            GRanges(
                              seqnames = unique(seqnames(wins)),
                              IRanges(
                                start = min(c(start(wins), start(gene_track))),
                                end = max(c(end(wins), end(gene_track)))
                                  )
                              ),
                            minoverlap = 1)
  
  wins2_track <- DataTrack(wins2, name = "Windowed_Fst")
  ...
  plotTracks(list(itrack, gtrack, wins2_track, ht), groups = names(mcols(wins2)),type = c("b"))
}

png(here("00_dashboard/figures","Pgr_Gviz_macro.png"), res = 300, width = 3500, height = 1500)
vis_goi_macro(ens_gene_id = "ENSMUSG00000031870", pop="DUC", bp_expand=5000)
dev.off()

```

```{r visualize_per_snp_Fst_Pgr_and_export_macro, eval = FALSE}

vis_goi_micro <- function(ens_gene_id, pos_snps, from_bp_adj, to_bp_adj){
  ...
  gene_track <- GeneRegionTrack(TxDb.Mmusculus.UCSC.mm10.ensGene, 
                                names = "Gene Model",
                                chromosome=unique(seqnames(query_genes)), 
                                start=start(query_genes)[1], 
                                end=end(query_genes)[length(query_genes)],
                                geneSymbol = TRUE, showId=TRUE)
  ...
  # define ideogram for chromosome
  itrack <- IdeogramTrack(genome = "mm10", chromosome = as.character(unique(seqnames(query_genes))))
  
  # track for genomic positions
  gtrack <- GenomeAxisTrack()

  # plot tracks
  plotTracks(list(itrack, gtrack, ht_snp), 
           groups = names(mcols(wins2)),
           type = c("b"), 
           from = start(query_genes)-from_bp_adj, 
           to = start(query_genes)+to_bp_adj)

  }


vis_goi_micro(ens_gene_id = "ENSMUSG00000031870", pos_snps = 8901222, from_bp_adj = 400, to_bp_adj = 3000) 

```

```{r make_frq_pie_chart, eval = FALSE}
make_frq_pie_chart <- function(chr, pos, hom_ref_gt, hom_alt_gt, het_gt, rsid, ref_allele, alt_allele){
...
make_frq_pie_chart(chr = 9, pos = 8901222, hom_ref_gt = "G/G", hom_alt_gt = "A/A", het_gt = "G/A", rsid = "rs16808511", ref_allele = "G", alt_allele = "A")
dev.off()
```
...
vis_goi_macro(ens_gene_id = "ENSMUSG00000026812", pop="DUK", bp_expand=5000)
dev.off()
```
