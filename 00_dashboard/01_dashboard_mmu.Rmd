...
cts_final_vcf_summ <- cts_final_vcf %>% 
  group_by(indel_class) %>% 
  summarise(total = sum(COUNT)) %>% 
  janitor::adorn_totals() %>% 
  tibble::column_to_rownames("indel_class")

p1 <- ggplot(cts_final_vcf, aes(x = LENGTH, y = COUNT)) +
  geom_bar(stat = "identity") +
  theme_bw(base_size = 15) +
  ggtitle(
    label = "INDEL counts by length"
    )
...
p3 <- cts_final_vcf %>% 
  ...
  ggplot(aes(x = indel_class, y = n_indels)) +
    ...
    #theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
    ggtitle(
      label = "INDEL counts by INDEL-class",
      subtitle = paste0("Total=",
                      cts_final_vcf_summ["Total",],
                      " (ins=",cts_final_vcf_summ["insertion",],
                      ", del=", 
                      cts_final_vcf_summ["deletion",], ")")
      )
...
indels_per_pop <- here("batches123_04_final_VCF/output") %>% 
  ...
  mutate(line = factor(line, levels = c("DUK","DUC","DU6","DU6P","DUhLB","FZTDU"))) %>% 
  arrange(line) %>% 
  select(contains(c("line","INDELS"))) %>% 
  # add counts ins and del per pop
  # extracted through: 'vcftools --hist-indel-len'
  inner_join(
    by = c("line"="pop"),
    lapply(c("DUK","DUC", "DU6", "DU6P","DUhLB", "FZTDU"), function(pop){
      paste0("cohort_biallelicINDELs_VQSR99_PASS_withmissingness.filtered.",pop,".indel.hist") %>% 
        here("batches123_04_final_VCF/output/",.) %>% 
        vroom() %>% 
        mutate(pop = pop) 
      }) %>% 
      bind_rows() %>% 
      mutate(indel_class = ifelse(LENGTH > 0, "insertion","deletion")) %>% 
      group_by(pop, indel_class) %>% 
      summarise(total = sum(COUNT)) %>% 
      reshape2::dcast(pop ~ indel_class, value.var = "total") %>% 
      janitor::adorn_totals(where = "col")
    
    ) %>% 
  dplyr::select(line, TOTAL_INDELS, insertion, deletion)
...
```{r vis_upster_and_export, eval = FALSE} 
...
### pct_in_FZTDU
```{r tabulate_sel_indels_in_fztdu}

data.frame(
  line = c("DUK","DUC","DU6","DU6P","DUhLB","FZTDU"),i
  set_size = c(
    sum(set_dat_bool$duk == 1),
    sum(set_dat_bool$duc == 1),
    sum(set_dat_bool$du6 == 1),
    sum(set_dat_bool$du6p == 1),
    sum(set_dat_bool$duhlb == 1),
    sum(set_dat_bool$fztdu == 1)
  ),
  intersection_sel_x_fztdu = c(
    sum(set_dat_bool$duk == 1 & set_dat_bool$fztdu == 1),
    sum(set_dat_bool$duc == 1 & set_dat_bool$fztdu == 1),
    sum(set_dat_bool$du6 == 1 & set_dat_bool$fztdu == 1),
    sum(set_dat_bool$du6p == 1 & set_dat_bool$fztdu == 1),
    sum(set_dat_bool$duhlb == 1 & set_dat_bool$fztdu == 1),
    sum(set_dat_bool$fztdu == 1 & set_dat_bool$fztdu == 1)
    )
) %>% 
  mutate(pct_sel_in_fztdu = (intersection_sel_x_fztdu / set_size ) * 100) %>% 
  kable(digits = 2) %>% 
  kable_styling(full_width = F)
