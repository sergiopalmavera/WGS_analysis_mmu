...
Overlap with WGS SNP set
```{r overlap_SNP_set}

wgs_snps <- here("batches123_04_final_VCF/output/cohort_biallelicSNPs_VQSR95_PASS_withmissingness.filtered.SNPids") %>% 
  vroom(col_names = FALSE, col_types = c("X1"="c"))

ov_wgs_snp_gigamuga <- wgs_snps %>% 
  inner_join(Marker_Giga_MUGA_buid_37_to_38, by = c("X1"="build38_chr", "X3"="build38_pos"))


wgs_snps %>% nrow()

Marker_Giga_MUGA_buid_37_to_38 %>% nrow()

ov_wgs_snp_gigamuga %>% nrow()
```

Little over 1/4 off gigamuga markers also appear in WGS-SNPs.
...
### How many WGS-SNP-sites remain?

```{r}

ov_wgs_snps_gigamuga_x_RDDq15q85_DUK <-  inner_join(wgs_snps, gigamuga_x_RDDq15q85_DUK, by = c("X1"="seqnames", "X3"="start"))
nrow(ov_wgs_snps_gigamuga_x_RDDq15q85_DUK)

ov_wgs_snps_gigamuga_x_RDDq15q85_DUC <- inner_join(wgs_snps, gigamuga_x_RDDq15q85_DUC, by = c("X1"="seqnames", "X3"="start"))
nrow(ov_wgs_snps_gigamuga_x_RDDq15q85_DUC)
```
