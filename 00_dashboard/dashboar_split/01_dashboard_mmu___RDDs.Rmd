...
```{r apply_function_to_find_distinct_REDs_aka_RDDs}
...
```{r export_RDDs, eval = FALSE}

saveRDS(distinct_windows_list, here("00_dashboard/data/RDDs_q10_vs_q95.rds"))
...
```{r export_RDD_SNP_data, eval=FALSE}

snps_in_RDDs %>% lapply(as.data.frame) %>% bind_rows(.id = "line") %>% write.csv("00_dashboard/data/snps_in_RDDs.csv")
...
RDD_genes2 <- lapply(1:length(RDD_genes), function(i){ 
  ...
  # do BM quary
  mart_res <- getBM(attributes = c("ensembl_gene_id","mgi_symbol","name_1006","namespace_1003","go_id"), 
                  filters = "ensembl_gene_id", 
                  values = x$mcols.gene_id,
                  mart = mart) %>% 
              filter(namespace_1003 == "biological_process") %>% 
              mutate(gobp_goid = paste0(name_1006,"(",go_id,")"))
  ...
  mart_res$ensembl_gene_id %>% 
    ...
    lapply(function(y){
      ...
      gobps_goids <- mart_res %>% 
        filter(ensembl_gene_id == y) %>%
        dplyr::select(gobp_goid) %>% 
        unlist() %>% 
        paste(collapse = "; ")
    ...
    dplyr::select(contrast, ensembl_gene_id,mgi_symbol,seqnames, start, end, width, strand, gene_biotype, GOBP,GOID,GOBP_GOID) 
})
...
colrs <- c("#7570B3","#66A61E", "#1B9E77", "#D95F02", "#E7298A", "grey")
names(colrs) <- c("DUK","DUC","DU6","DU6P","DUhLB", "FERT")  
    
```

```{r vis_distr_rdd_fst_vals, out.width = "70%", fig.align = "center"} 

rdd_win_dat <- lapply(names(distinct_windows_list), function(pop){
  
  wins <- distinct_windows_list[[pop]] %>% mutate(tmp = paste(CHROM, BIN_START, BIN_END, sep = "_")) %>% dplyr::select(tmp)
  
  win_fst_sel_vs_ctrl_prep %>% 
    mutate(tmp = paste(CHROM, BIN_START, BIN_END, sep = "_")) %>%
    inner_join(wins, by = "tmp") %>% 
    mutate(rdd_for_pop = pop)
  
  }) %>% 
  bind_rows() %>% 
  mutate(rdd_for_pop = factor(rdd_for_pop, levels = names(distinct_windows_list))) %>% 
  ungroup() %>% 
  dplyr::rename(pop_vs_fztdu = contrast) %>% 
  mutate(pop_vs_fztdu = str_remove(pop_vs_fztdu, "_vs_FZTDU"))


p <- ggplot(rdd_win_dat, aes(x = MEAN_FST, fill = pop_vs_fztdu)) +
  ...
  ylab(NULL) + xlab(expression(Fst[Selected - FZTDU]) ) +
  scale_x_continuous(expand = c(0,0.00), breaks = seq(0,1,0.2)) +
  facet_wrap(~rdd_for_pop, ncol = 1, strip.position = "left", scales = "free_y")

print(p)
```

```{r export_for_ms, eval = FALSE} 

png(here("00_dashboard/for_manuscript/rdd_win_distr.png"), res = 300, height = 2500, width = 1200, units = "px")
print(p)
dev.off()

# split for fertility lines
png(here("00_dashboard/for_manuscript/rdd_win_distr_duk_duc_fert.png"), res = 300, height = 1000, width = 1200, units = "px")

rdd_win_dat %>% 
  filter(rdd_for_pop %in% c("DUK","DUC","FERT")) %>% 
  ggplot(rdd_win_dat, mapping =  aes(x = MEAN_FST, fill = pop_vs_fztdu)) +
    geom_density(alpha = 0.6, size = 0.2) +
    theme_bw() + # theme_bw(base_size = 12) 
    theme(panel.grid = element_blank(), 
          axis.ticks.y = element_blank(), 
          axis.text.y = element_blank(),
          legend.position = "right", legend.title = element_blank(),
          axis.text.x = element_text(size = 8),
          strip.text = element_text(size = 8)) + # strip.text = element_text(size = 12))
    scale_fill_manual(values = colrs) +
    scale_y_continuous(expand = c(0,0.1)) +
    ylab(NULL) + 
    xlab(expression(F[ST(trait_selected - FZTDU)]) ) +
    scale_x_continuous(expand = c(0,0.00), breaks = seq(0,1,0.2)) +
    facet_wrap(~rdd_for_pop, ncol = 1, strip.position = "left", scales = "free_y")

dev.off()

# split for rest of lines

png(here("00_dashboard/for_manuscript/rdd_win_distr_du6_du6p_duhlb.png"), res = 300, height = 1000, width = 1200, units = "px")
rdd_win_dat %>% 
  filter(rdd_for_pop %in% c("DU6","DU6P","DUhLB")) %>% 
  ggplot(rdd_win_dat, mapping =  aes(x = MEAN_FST, fill = pop_vs_fztdu)) +
    geom_density(alpha = 0.6, size = 0.2) +
    theme_bw() +
    theme(panel.grid = element_blank(), 
...
```{r prepare_data_mahattans, eval = FALSE} 
...
```{r prep_rdd_wins, eval = FALSE} 

rdd_wins <- semi_join(manhattan_dat, distinct_windows_list %>% bind_rows(), by = c("contrast","CHROM","BIN_START","BIN_END")) %>% 
  ungroup() %>% mutate(pop_vs_fztdu = str_remove(contrast, "_vs_FZTDU")) %>% dplyr::select(-contrast) %>% 
  mutate(pop_vs_fztdu = factor(pop_vs_fztdu, c("DUK","DUC","DU6","DU6P","DUhLB","FERT")))

rest_wins <- anti_join(manhattan_dat, distinct_windows_list %>% bind_rows(), by = c("contrast","CHROM","BIN_START","BIN_END")) %>% 
    ungroup() %>% mutate(pop_vs_fztdu = str_remove(contrast, "_vs_FZTDU")) %>% dplyr::select(-contrast) %>% 
    mutate(pop_vs_fztdu = factor(pop_vs_fztdu, c("DUK","DUC","DU6","DU6P","DUhLB","FERT")))

q_fst  <- q_fst %>% ungroup() %>% mutate(pop_vs_fztdu = str_remove(contrast, "_vs_FZTDU")) %>% dplyr::select(-contrast) %>% 
  mutate(pop_vs_fztdu = factor(pop_vs_fztdu, c("DUK","DUC","DU6","DU6P","DUhLB","FERT")))
```

```{r create_genomewide_manhattan_with_rdds, eval = F} 
# run this once to create genomewide manhattan and export

# Manhattan of win-fst scores
manh_rdd_loci <- rest_wins %>% 
  #.[sample(nrow(.), 10000),] %>%
  # plot main data (rest)
  ggplot(data = ., mapping = aes(x = genomic_index, y = MEAN_FST, color = chr_color)) +
    geom_point(alpha = 0.5) +
    # plot rdd win data
    geom_point(rdd_wins, mapping=aes(x = genomic_index, y = MEAN_FST), color = "red", alpha = 1) +
    # visual theme
    ...
    ggtitle("Windowed Fst - Selected vs FZTDU") +
    # facet
    facet_wrap(~pop_vs_fztdu, ncol = 1, strip.position = "right", scales = "free") +
    # hline
    geom_hline(filter(q_fst, Quantile %in% c("q10","q95")), mapping = aes(yintercept = Fst, color = Quantile), linetype = "dashed")

```

```{r export_manhattan_plot_with_rdd_wins, eval = FALSE}
png(here("00_dashboard/for_manuscript/manhattan_rdd_win.png"), res = 300, height = 3000, width = 4000, units = "px")
manh_rdd_loci
dev.off()
```

```{r display_rdd_win_loci_manhattan, out.width="90%", fig.align="center"}

include_graphics(here("00_dashboard/for_manuscript/manhattan_rdd_win.png"))

```

```{r export_manhattan_plot_with_q99_q95_for_ms, eval = FALSE}
# function to avoid overlap between chrs
make_manhattan_chr_facets <- function(pop){
  mdat <- manhattan_dat %>% 
    ungroup() %>% 
    mutate(pop_vs_fztdu = str_remove(contrast, "_vs_FZTDU")) %>% 
    dplyr::select(-contrast) %>% 
    mutate(pop_vs_fztdu = factor(pop_vs_fztdu, c("DUK","DUC","DU6","DU6P","DUhLB","FERT"))) %>% 
    filter(pop_vs_fztdu == pop)
  qdat <- q_fst %>% 
    filter(Quantile %in% c("q95","q99"), pop_vs_fztdu == pop)
  
  contr <- paste0("(",pop,"-FZTDU)")
  ...
  p_res <- mdat %>% 
    ...
    ggplot(data = ., mapping = aes(x = genomic_index, y = MEAN_FST, color = chr_color)) +
      ...
      scale_y_continuous(breaks = seq(0,1,0.5), expand = c(0.01, 0.01)) +
      # labs and title
      xlab(NULL) +
      ylab( bquote( Fst[.(contr)]  ) ) +
      # facet
      facet_wrap(~CHROM, nrow = 1, scales = "free_x", strip.position = "bottom") +
      # hline
      geom_hline(qdat, mapping = aes(yintercept = Fst, color = Quantile), linetype = "dashed") 
  
  print(p_res)
}


png(here("00_dashboard/for_manuscript/manhattan_q95_q99.png"), res = 300, height = 3000, width = 4000, units = "px")
gridExtra::grid.arrange(make_manhattan_chr_facets("DUK"), 
                        make_manhattan_chr_facets("DUC"),
                        make_manhattan_chr_facets("DU6"), 
                        make_manhattan_chr_facets("DU6P"),
                        make_manhattan_chr_facets("DUhLB"),
                        make_manhattan_chr_facets("FERT"),
                        ncol = 1)
dev.off()
...
### DUK_genes 
```{r} 
here("00_dashboard/data/RDD_genes/RDD_genes_DUK.csv") %>% read.csv() %>% dplyr::select(-X,-GOID,-GOBP_GOID) %>% datatable(options=list(paging = FALSE))
```

### DUC_genes 
```{r}
here("00_dashboard/data/RDD_genes/RDD_genes_DUC.csv") %>% read.csv() %>% dplyr::select(-X,-GOID,-GOBP_GOID) %>% datatable(options=list(paging = FALSE))
```

### DU6_genes 
```{r}
here("00_dashboard/data/RDD_genes/RDD_genes_DU6.csv") %>% read.csv() %>% dplyr::select(-X,-GOID,-GOBP_GOID) %>% datatable(options=list(paging = FALSE))
```

### DU6P_genes
```{r} 
here("00_dashboard/data/RDD_genes/RDD_genes_DU6P.csv") %>% read.csv() %>% dplyr::select(-X,-GOID,-GOBP_GOID) %>% datatable(options=list(paging = FALSE))
```

### DUhLB_genes 
```{r}
here("00_dashboard/data/RDD_genes/RDD_genes_DUhLB.csv") %>% read.csv() %>% dplyr::select(-X,-GOID,-GOBP_GOID) %>% datatable(options=list(paging = FALSE))
```

### FERT_genes 
```{r}
here("00_dashboard/data/RDD_genes/RDD_genes_FERT.csv") %>% read.csv() %>% dplyr::select(-X,-GOID,-GOBP_GOID) %>% datatable(options=list(paging = FALSE))
```
...
```{r for_ms_merge_rdd_wins_redo_rdd_genes_loci_vert, eval = FALSE}
# import chr length information
chr_len <- read.table(here("reference_genome_ensembl/Mus_musculus.GRCm38.dna.primary_assembly.dict"), header = F, skip = 1) %>% 
  dplyr::select(V2,V3) %>% 
  mutate(V2 = str_remove(V2,"SN:"),
         start = 1,
         V3 = str_remove(V3,"LN:") %>% as.numeric()) %>% 
  dplyr::rename(chrom = V2, end = V3) %>% 
  filter(chrom  %in% c(1:19,"X")) %>% 
  dplyr::select(chrom,start,end) %>% 
  mutate(chrom = factor(chrom, levels = c(1:19,"X"))) %>% 
  arrange(chrom) %>% 
  mutate(cum_sum = cumsum(end), diff = cum_sum - end, new_start = start + diff, new_end = new_start + end, 
         center = new_start + (new_end - new_start)/2)

plot_manhattan_vertical_reduced <- function(pop,   x = 3, x_lim = 200, force_repel = 1){
  
  # prepare contrast name
  contr <- paste0(pop,"_vs_FZTDU")

  # get rdd windows 
  rdd_pop <- distinct_windows_list[[pop]]
  
  # reduce windows, add chr offset
  rdd_pop_red <- rdd_pop %>% 
    dplyr::rename(seqnames=CHROM, start=BIN_START, end=BIN_END) %>% 
    makeGRangesFromDataFrame(keep.extra.columns = TRUE) %>% 
    reduce(min.gapwidth = 1000000) %>% 
    as.data.frame() %>% 
    # add offset
    inner_join(dplyr::select(chr_len, chrom,  diff), by = c("seqnames"="chrom")) %>% 
      mutate(new_start = start + diff, new_end = end + diff) %>% 
    mutate(center = new_start + (new_end - new_start)/2)
  
  # Find genes in rdd wins
  ov <-findOverlaps(
    query = rdd_pop %>% dplyr::rename(seqnames=CHROM, start=BIN_START, end=BIN_END) %>% makeGRangesFromDataFrame(),
    subject = mmu_gene_set_gr,
    minoverlap = 1
    )
  gn_hits <- mmu_gene_set_gr[subjectHits(ov)]
  
  # add rdd-genes to reduced rdd windows
  ov2 <- findOverlaps(query = rdd_pop_red %>% makeGRangesFromDataFrame(keep.extra.columns = TRUE), 
                      subject = gn_hits, 
                      minoverlap = 1)
  
  gns_mapped <- bind_cols(
    rdd_pop_red[queryHits(ov2),], 
    gn_hits[subjectHits(ov2)] %>% 
      as.data.frame() %>% 
      dplyr::select(mcols.gene_symbol)
    ) %>% 
    group_by(center) %>% 
    summarise(genes = paste(unique(mcols.gene_symbol), collapse = "\n")) # add line break

  set.seed(458)
  
  # make base plot
  ggplot() +
    geom_rect(chr_len, mapping = aes(xmin = 0, xmax = 2, ymin = new_start, ymax = new_end, fill = chrom), 
              show.legend = FALSE)  +
    scale_fill_manual(values = rep(c("black","grey"), 10)) +
    #xlim(0,x_lim) +
    scale_x_continuous(limits = c(0,x_lim)) +
    theme_void(base_size = 12) +  
    theme(plot.title = element_text(hjust = 0)) +
    geom_text(chr_len, mapping = aes(x = x-x, y = center, label = chrom), hjust = 1, size = 5) +
    
    # add rdd merged regions
    geom_rect(rdd_pop_red, mapping = aes(xmin = 0, xmax = 2, ymin = new_start, ymax = new_end), color = "red") +
    
    # add a tag signal
    geom_segment(rdd_pop_red, mapping = aes(x = 2, xend = x, y = center, yend = center), color = "red") +
    geom_point(rdd_pop_red, mapping = aes(x = x, y = center), color = "red") +
    
    # add gene lables
    geom_label_repel(gns_mapped, mapping = aes(x = x, y = center, label = genes),
                     xlim = c(x*2,NA), force = force_repel, max.iter	= 10000000,
                     segment.alpha = 0.6)  +
    scale_y_reverse() +
    ggtitle(contr)
}

png(here("00_dashboard/for_manuscript/manhattan_vert_red_DUK.png"), width = 2000, height = 3500, units = "px",res = 300)
plot_manhattan_vertical_reduced("DUK", force_repel = 10) #
dev.off()

png(here("00_dashboard/for_manuscript/manhattan_vert_red_DUC.png"), width = 2000, height = 2000, units = "px",res = 300)
plot_manhattan_vertical_reduced("DUC") 
dev.off()

png(here("00_dashboard/for_manuscript/manhattan_vert_red_DU6.png"), width = 2000, height = 2000, units = "px",res = 300)
plot_manhattan_vertical_reduced("DU6") 
dev.off()

png(here("00_dashboard/for_manuscript/manhattan_vert_red_DU6P.png"), width = 2000, height = 2000, units = "px",res = 300)
plot_manhattan_vertical_reduced("DU6P") 
dev.off()

png(here("00_dashboard/for_manuscript/manhattan_vert_red_DUhLB.png"), width = 2000, height = 3500, units = "px",res = 300)
plot_manhattan_vertical_reduced("DUhLB", force_repel = 10) #
dev.off()

png(here("00_dashboard/for_manuscript/manhattan_vert_red_FERT.png"), width = 2000, height = 2000, units = "px",res = 300)
plot_manhattan_vertical_reduced("FERT") 
dev.off()
...
- Mostly null results using RDD genes (q10vs_q95).

- RDDs were augmented because RDD-genes (although informative) were too few. ORA was not very informative. Mostly not significant results were returned.

- Different lists were tested (q10vs_q95, q10vs_q90, q15vs_q85, q20vs_q80, q25vs_q75 and (these were added later) rdd_q10_q85, q25vs_q85)
...
# these lists were added later as requested by jenni
rdd_q10_q85 <- get_RDDs_and_RDDgenes(q_low = "q10", q_high = "q85")
rdd_q25_q85 <- get_RDDs_and_RDDgenes(q_low = "q25", q_high = "q85")
saveRDS(rdd_q25_q85, here("00_dashboard/data/rdd_q25_q85.rds")) # save for RDDs_expanded
```
...
# rdd_q10_q85
WebGestaltR_GOBP_wrapper(pop = "DUK", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 88
WebGestaltR_GOBP_wrapper(pop = "DUC", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 51
WebGestaltR_GOBP_wrapper(pop = "DU6", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 40
WebGestaltR_GOBP_wrapper(pop = "DU6P", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 36
WebGestaltR_GOBP_wrapper(pop = "DUhLB", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 97
WebGestaltR_GOBP_wrapper(pop = "FERT", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # sig - 35

WebGestaltR_KEGG_wrapper(pop = "DUK", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # sig - 88
WebGestaltR_KEGG_wrapper(pop = "DUC", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 51
WebGestaltR_KEGG_wrapper(pop = "DU6", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 40
WebGestaltR_KEGG_wrapper(pop = "DU6P", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 36
WebGestaltR_KEGG_wrapper(pop = "DUhLB", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # sig - 97
WebGestaltR_KEGG_wrapper(pop = "FERT", rdd_gene_dat = rdd_q10_q85, q_low = "q10", q_high = "q85") # notsig - 35

# rdd_q25_q85
WebGestaltR_GOBP_wrapper(pop = "DUK", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #not sig - 256
WebGestaltR_GOBP_wrapper(pop = "DUC", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #not sig - 232
WebGestaltR_GOBP_wrapper(pop = "DU6", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #sig - 170
WebGestaltR_GOBP_wrapper(pop = "DU6P", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #sig-187
WebGestaltR_GOBP_wrapper(pop = "DUhLB", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #notsig -346
WebGestaltR_GOBP_wrapper(pop = "FERT", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #notsig - 204

WebGestaltR_KEGG_wrapper(pop = "DUK", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #sig - 256
WebGestaltR_KEGG_wrapper(pop = "DUC", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #notsig - 232
WebGestaltR_KEGG_wrapper(pop = "DU6", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #not sig - 170
WebGestaltR_KEGG_wrapper(pop = "DU6P", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #not sig - 187
WebGestaltR_KEGG_wrapper(pop = "DUhLB", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") #not sig - 346
WebGestaltR_KEGG_wrapper(pop = "FERT", rdd_gene_dat = rdd_q25_q85, q_low = "q25", q_high = "q85") # sig - 204
```
...
# these lists were added later as requested by jenni
rdd_q10_q85$rdd_genes %>% lapply(as.data.frame) %>% bind_rows(.id = "pop") %>% write.csv(here("00_dashboard/data/WebGestaltR_RDD_q10_q85","rdd_genes_q10_q85.csv"))
rdd_q25_q85$rdd_genes %>% lapply(as.data.frame) %>% bind_rows(.id = "pop") %>% write.csv(here("00_dashboard/data/WebGestaltR_RDD_q25_q85","rdd_genes_q25_q85.csv"))
