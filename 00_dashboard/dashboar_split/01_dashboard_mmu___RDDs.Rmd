...
# they were not used for ORA
rdd_q10_q85 <- get_RDDs_and_RDDgenes(q_low = "q10", q_high = "q85")
rdd_q25_q85 <- get_RDDs_and_RDDgenes(q_low = "q25", q_high = "q85")


rdd_q10_q85$rdd_genes %>% lapply(as.data.frame) %>% bind_rows(.id = "pop") %>% write.csv(here("00_dashboard/data/RDD_genes","rdd_genes_q10_q85.csv"))
rdd_q25_q85$rdd_genes %>% lapply(as.data.frame) %>% bind_rows(.id = "pop") %>% write.csv(here("00_dashboard/data/RDD_genes","rdd_genes_q25_q85.csv"))
```
...
# function1 ???
rdd_gene_lists <- here("00_dashboard/data/RDD_genes",paste0("RDD_genes_",c("DUK","DUC","DU6","DU6P","DUhLB","FERT"),".csv"))
#term_col <- "GOBP"
#id_col <- "GOID"
term_id_col <- "GOBP_GOID"
gene_col <- "mgi_symbol"
contrast_col <- "contrast"
#

gene_terms <- rdd_gene_lists %>% 
  lapply(function(fl){
    fl %>% vroom() 
  }) %>% 
  bind_rows()
...
gene_terms2 <- gene_terms %>%
  apply(1,function(row_i){
    
    data.frame(contrast = row_i[contrast_col] %>% as.character(), 
               gene = row_i[gene_col] %>% as.character(), 
               term_id = row_i[term_id_col] %>% str_split(";") %>% unlist() %>% str_trim())
...
dat <- gene_terms2 #cols must be contrast, gene, term_id

#

ma <- dat %>% 
  filter(contrast == contrast, !is.na(term_id)) %>% 
  dplyr::select(gene, term_id) %>%
  separate(col = term_id, sep = "[(]GO:", into = c("term","tmp2")) %>% 
  mutate(go_id = str_remove(tmp2,"[)]") %>% paste0("GO:",.)) %>% 
  mutate(tmp = 1) %>% 
  unique() %>% 
  reshape2::dcast(go_id ~ gene, value.var = "tmp", fun.aggregate = length) %>% 
  tibble::column_to_rownames("go_id") %>% 
  as.matrix()


# function 3 ???
pheatmap::pheatmap(ma, cluster_rows = TRUE, cluster_cols = TRUE)
