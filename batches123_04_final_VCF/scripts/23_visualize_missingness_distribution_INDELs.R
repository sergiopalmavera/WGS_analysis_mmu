...
# make a function to extract number of missing samples per pop at each INDEL
...
png("../figures_tables/miss_cts_INDEL.png", width = 3000, height = 2000, units = "px", res = 300)
miss_cts2 %>%
  # Visualize
  ggplot(aes(x = miss_counts, after_stat(count), color = Line)) +
    ...
    xlab("missing counts") +
    ylab("INDEL counts") +
    ggtitle("Distribution of Number of Samples Missing per INDEL")
dev.off()    


png("../figures_tables/miss_cts_facet_INDEL.png", width = 3000, height = 3000, units = "px", res = 300)
miss_cts2 %>%
  # Visualize
