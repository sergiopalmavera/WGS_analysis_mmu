...
```{r global_fst_pairwise, include=F}
...
names(snpeff_data) <- fls %>% 
  str_remove("cohort_biallelicSNPs_VQSR95_PASS_AddedMissingness.recode.filtered.ann_") %>% 
  str_remove(".csv")
...
snpeff_data$change_rate_by_chr %>% 
  datatable(rownames = F, options = list(columnDefs = list(list(className = 'dt-center', targets = "_all"))))
```
...
```{r}
include_graphics(here("batches123_08_LDD_SFS/figures","allele_frq_state.png"))
```
