...
# convert VCF to GDS
vcf <- "../../batches123_04_final_VCF/output/cohort_biallelicSNPs_VQSR95_PASS_withmissingness.filtered.vcf"

seqVCF2GDS(vcf, "../data/cohort_biallelicSNPs_VQSR95_PASS_withmissingness.filtered.gds")

# Import GDS data
genofile <- seqOpen("../data/cohort_biallelicSNPs_VQSR95_PASS_withmissingness.filtered.gds")

# PCA
pca <- snpgdsPCA(genofile)

# Export table of PC percentage
data.frame(PC=1:7, varprop = pca$varprop[1:7]*100) %>% write.csv("../data/pc_pct_full_snp_set.csv")
...
write.csv(tab, "../data/pc_eigenscores_full_snp_set.csv")
