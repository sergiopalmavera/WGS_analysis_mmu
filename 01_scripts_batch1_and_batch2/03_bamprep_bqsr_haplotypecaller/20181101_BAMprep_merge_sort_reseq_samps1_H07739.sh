...
do
	...
	# Deduplicate and remove input if succesful #-> IMPORTANT: this step failed. Script only produced merged.sorted.bam for H07739
	...
	time java -jar $PICARD/picard.jar MarkDuplicates \
		...
		M=$RES/merged/$samp.merged.sorted.dedup.metrics.txt \ #---> this directory was wrong (merged should'nt've been there)
		TMP_DIR=$TMP/tmp/$samp
