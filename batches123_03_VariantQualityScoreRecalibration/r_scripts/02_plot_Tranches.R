...
targetTITV = 2 #as.numeric(args[2])
tranchesFile = "../output/cohort_biallelicSNPs_2.tranches" #"TestTranchesPlot" #args[1]
suffix = paste0("Modified_targetTITV_",targetTITV)
targetSensitivity = c(90,95,99,99.9,100) #as.numeric(args[3])
suppressLegend = 0 #! is.na(args[4])
...
data2 <- data2 %>% filter(targetTruthSensitivity %in% targetSensitivity)
...
density=c(20, -1, -1, 20)
outfile = paste(tranchesFile, "_",suffix,".pdf", sep="")
...
if ( ! suppressLegend ) 
  ...
  legend(2000, 10, c('Cumulative TPs','Tranch-specific TPs', 'Tranch-specific FPs', 'Cumulative FPs' ), fill=cols, density=density, bg='white', cex=1.25)
...
# Make a table with FP fraction
FP_fraction <- (numNewBad + numPrevBad) / (numPrevGood + numNewGood + numNewBad + numPrevBad)
names(FP_fraction) <- sort(targetSensitivity, decreasing = TRUE)
FP_fraction <- round(FP_fraction, 3)
FP_fraction
