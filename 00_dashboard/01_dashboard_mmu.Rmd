...
```{r message_to_redirect_to_Manhattans_figures_files}
data.frame(
  file = list.files(here("00_dashboard/figures"), pattern = ".png$") %>% 
    .[grep("fst_chr",.)] %>% 
    .[-grep("RED_genes_HIGH_labs",.)] %>% 
    .[-grep("z_fst",.)] %>% 
    sapply(function(x) file.path("00_dashboard/figures",x))
) %>% 
...
```{r message_to_redirect_to_zManhattans_figures_files}
data.frame(
  file = list.files(here("00_dashboard/figures"), pattern = ".png$") %>% 
    .[grep("fst_chr",.)] %>% 
    .[-grep("RED_genes_HIGH_labs",.)] %>% 
...
ma_mean_fst_genes <- read.csv(here("00_dashboard/data/mean_fst_candidate_genes.csv")) %>% dplyr::select(-X)
...
get_distinct_genes <- function(cont1, rest_cont){
  
  #cont1="DUK_vs_FZTDU"
  #rest_cont=c("DUC_vs_FZTDU","DU6_vs_FZTDU","DU6P_vs_FZTDU","DUhLB_vs_FZTDU")

  cond1 <- ma_mean_fst_genes[,cont1] >= 0.7
  
  cond2 <-  ma_mean_fst_genes[,rest_cont] %>% apply(1, function(x) all(x < 0.5) )
  
  idx <- cbind(cond1,cond2) %>% apply(1, all)
  
  ma_mean_fst_genes[idx,] %>% as.data.frame()

}

most_distinct_diff_genes <- list(
  duk = get_distinct_genes("DUK_vs_FZTDU", c("DUC_vs_FZTDU","DU6_vs_FZTDU","DU6P_vs_FZTDU","DUhLB_vs_FZTDU")),
  duc = get_distinct_genes("DUC_vs_FZTDU", c("DUK_vs_FZTDU","DU6_vs_FZTDU","DU6P_vs_FZTDU","DUhLB_vs_FZTDU")),
  du6 = get_distinct_genes("DU6_vs_FZTDU", c("DUC_vs_FZTDU","DUK_vs_FZTDU","DU6P_vs_FZTDU","DUhLB_vs_FZTDU")),
  du6p = get_distinct_genes("DU6P_vs_FZTDU", c("DUC_vs_FZTDU","DU6_vs_FZTDU","DUK_vs_FZTDU","DUhLB_vs_FZTDU")),
  duhlb = get_distinct_genes("DUhLB_vs_FZTDU", c("DUC_vs_FZTDU","DU6_vs_FZTDU","DU6P_vs_FZTDU","DUK_vs_FZTDU"))
)
...
mat_distinct %>% 
  ...
  as.matrix() %>% 
  .[,c("DUK_vs_FZTDU","DUC_vs_FZTDU","DU6_vs_FZTDU","DU6P_vs_FZTDU","DUhLB_vs_FZTDU")] %>% 
...
read.csv(here("00_dashboard/data/mean_fst_candidate_genes.csv")) %>% 
  inner_join(
  ...
  unique() %>%
  datatable(options = list(pageLength = 100), caption = "Mean Fst Candidate Genes (genes with a high-impact SNP)")
