...
```{r prepare_data_for_upsetr, eval = F, include=F}
...
### Mean Genomewide Coverage (supp. fig. 1)
...
### Percentage of Genome Territory Covered (supp. fig. 2)
...
### Per-sample metrics (supp. table 1)
```{r}
# raw nr of reads
d1 <- 
  # raw nr ofreads
...
#write.csv(per_sample_metrics, here("00_dashboard/for_manuscript/per_sample_metrics.csv"))
```

### Summary metrics (supp. table 2-5)
```{r}
per_sample_metrics %>% 
  ...
  kable(caption = "Supplementary table 2", digits = 2) %>% 
...
per_sample_metrics %>% 
  ...
  kable(caption = "Supplementary table 3", digits = 2) %>% 
  kable_styling(full_width = F) %>%
...
per_sample_metrics %>% 
  ...
  kable(caption = "Supplementary table 4", digits = 2) %>% 
  kable_styling(full_width = F) %>%
...
per_sample_metrics %>% 
  ...
  kable(caption = "Supplementary table 5", digits = 2) %>% 
  kable_styling(full_width = F) %>%
...
### Number of Variants (supp. table 6)
```{r}
vcfs_main_summary <- bind_rows(
  ...
  knitr::kable(format.args = list(big.mark = ","), digits = 2,
               caption = "Supplementary table 6: SNPs by call set stage") %>% 
  kable_styling(full_width = F)
...
read.csv(
  ...
  kable(align = "l", digits = 2, 
        caption = "Supplementary table 7: Fraction of mouse line with at least N calls") %>% 
  kable_styling(full_width = F)
```



### SNPs per sample (supp. fig. 5)
```{r}
...
here("batches123_04_final_VCF/output") %>% 
  ...
  bind_rows() %>% 
  kable(caption = "Table 1: Per-Line SNP summary", digits = 2) %>% 
  kable_styling(full_width = F) %>% 
...
### Intersections minimal (supp. fig. 6)
...
### Intersection fixed SNPs (supp. fig. 7)
```{r}
...
### counts by genotype region (supp. table 10)
```{r}
snpeff_data$cts_by_geno_region %>% 
  datatable(rownames = F, options = list(pageLength = 25, columnDefs = list(list(className = 'dt-center', targets = "_all"))))
```
