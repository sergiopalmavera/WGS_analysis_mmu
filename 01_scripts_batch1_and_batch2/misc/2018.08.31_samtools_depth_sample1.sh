...
do
	echo "## SAMPLE $samp"	
	samtools view -F 0x400 $BAMDIR/$samp.merged.sorted.dedup.bqsr.bam > $TMP/$samp.tmp.bam 
	samtools depth -q 20 -Q 20 $TMP/$samp.tmp.bam > $TMP/$samp.merged.sorted.dedup.bqsr.cvg #Qq same as picard's CollectWgsMetrics
	rm $TMP/$samp.tmp.bam
