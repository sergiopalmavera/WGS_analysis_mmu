...
do
	...
	then
		...
		# Report sizes and remove temporary per lane bams 
		echo "Per-lane RG-BAM sizes sample $SAMP"
		ls -lh $TMP/$SAMP/*.RG.bam
		rm $TMP/$SAMP/*.RG.bam
		...
		# Report tmp file size and remove
		echo "Size $SAMP.merged.bam = $(ls -lh $TMP/$SAMP/$SAMP.merged.bam |awk '{print $5}')"
		...
		# Report size and Remove temporary merged bam
		echo "Size $SAMP.merged.sorted.bam = $(ls -lh $TMP/$SAMP/$SAMP.merged.bam |awk '{print $5}')"
		...
		# Report size and remove temporary file
		echo "Size $SAMP.merged.sorted.dedup.bam = $(ls -lh $TMP/$SAMP/$SAMP.merged.bam |awk '{print $5}')"
