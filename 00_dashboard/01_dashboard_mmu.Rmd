...
tabulate_genotypes_per_contrast <- function(pop){
  ...
  samps_pop <- sample_info %>% filter(Linie == pop) %>% .$sample_id
...
write.csv(RED_genes_all_mean_fst, here("00_dashboard/data/RED_gene_mean_fst_all_contrasts.csv"))
...
read.csv(
  here("00_dashboard/data/RED_gene_mean_fst_all_contrasts.csv")
  ) %>% 
  dplyr::select(-X) %>% 
  datatable(
    #options = list(pageLength = 300), 
    caption = "Mean Fst in RED Genes", 
    rownames = FALSE, 
    filter = "top"
    ) %>% 
  formatRound(
...
# export this table to avoid running this part again (it takes too long to complete)
write.csv(RED_genes_all_nsnps, here("00_dashboard/data/RED_gene_nsnp_all_contrasts.csv"))
```

```{r display_nsnps_RED_genes}
read.csv(
  here("00_dashboard/data/RED_gene_nsnp_all_contrasts.csv")
  ) %>% 
  dplyr::select(-X) %>% 
  datatable(
    #options = list(pageLength = 300), 
    caption = "Number of SNPs in RED Genes", 
    rownames = FALSE, 
    filter = "top"
    )
...
ma <- read.csv(here("00_dashboard/data/RED_gene_mean_fst_all_contrasts.csv")) %>% dplyr::select(-X)
...
#ma[gns,] %>% write.csv(here("00_dashboard/data/RED_gene_mean_fst_distinct.csv"))
...
RED_gene_mean_fst_distinct %>% 
  datatable(
    options = list(pageLength = 300), 
    caption = "Mean Fst distinct RED Genes", 
    rownames = FALSE, filter = "top"
    )

# export for sharing with colleagues
# RED_gene_mean_fst_distinct %>% write.csv(here("00_dashboard/data/RED_gene_mean_fst_distinct2.csv"))
...
lapply(1:length(RED_genes), function(i){
  gn_ids <- RED_genes[[i]] %>% 
              ...
              filter(gene_id2 %in% FL2_testis_DEGs_gene_symbols) %>% 
              unlist()
  names(gn_ids) <- NULL
  ...
  bind_rows() %>% 
  kable(caption = "RED genes x DEGs (FL2 testis)") %>% 
  kable_styling(full_width = F)
...
lapply(1:length(RED_genes), function(i){
  gn_ids <- RED_genes[[i]] %>% 
              ...
              dplyr::select(gene_id2) %>% 
              mutate(gene_id2 = str_trim(gene_id2)) %>% #just in case
              filter(gene_id2 %in% FL1_ovary_DEGs_gene_symbols) %>% 
              unlist()
  names(gn_ids) <- NULL
  gn_ids2 <- paste(gn_ids, collapse = " ; ")
  data.frame(contrast = names(RED_genes)[i], n_common_genes = length(gn_ids), gene_symbol = gn_ids2)
  ...
  bind_rows() %>% 
  kable(caption = "RED genes x DEGs (FL1 ovary)") %>% 
  kable_styling(full_width = F)
