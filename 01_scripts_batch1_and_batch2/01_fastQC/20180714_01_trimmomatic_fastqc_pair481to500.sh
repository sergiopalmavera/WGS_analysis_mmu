...
mkdir $OUTPUT/tmp.$FROM.$TO
mkdir $OUTPUT/tmp.$FROM.$TO/TRIMMO
mkdir $OUTPUT/tmp.$FROM.$TO/FASTQC
...
do
	...
	then
		...
		TMP=$OUTPUT/tmp.$FROM.$TO/${file%%_R*}
		mkdir -p $TMP
		...
		mv $TMP/TRIMMO/* $OUTPUT/tmp.$FROM.$TO/TRIMMO
		mv $TMP/FASTQC/* $OUTPUT/tmp.$FROM.$TO/FASTQC
		rm -rf $TMP
