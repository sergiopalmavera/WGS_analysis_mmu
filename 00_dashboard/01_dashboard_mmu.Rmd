...
```{r snpeff_genes, include=F}

snpeff_genes <- vroom(
  here("batches123_05_annotation/output/cohort_biallelicSNPs_VQSR95_PASS_withmissingness.filtered.summary.genes.txt"),
  skip = 1
  )
...
snpeff_snps_HIGH <- read.table(
  ...
  dplyr::rename(CHROM = V1, POS = V2, INFO = V3)
...
```{r}
snpeff_genes_HIGH <- snpeff_genes %>% filter(variants_impact_HIGH > 0) 

#
contr=RED_genes$DU6P_vs_FZTDU
