...
### Intersection 99th quantile windows
```{r instersection_q99th_windows, fig.width=10}
...
```{r upsetr_RED_genes, fig.width=10}
...
#png(here("00_dashboard/for_manuscript/supplementary_figure_2.png"), res = 300, height = 2000, width = 3000, units = "px")
...
### Intersection Gene Sets (protein coding)
```{r upsetr_RED_genes_protein_coding, fig.width=10}
...
#png(here("00_dashboard/for_manuscript/supplementary_figure_3.png"), res = 300, height = 2000, width = 3000, units = "px")
...
bind_rows(gobp_sig_top, kegg_sig) %>% 
  arrange(contrast, enrichmentRatio) %>% 
  mutate(
    description = factor(description, levels = .$description),
    contrast = factor(contrast, levels = c("DUK_vs_FZTDU","DU6_vs_FZTDU","DU6P_vs_FZTDU","DUhLB_vs_FZTDU"))
    ) %>% 
  ggplot(aes(x = enrichmentRatio, y = description, fill = contrast)) +
    geom_bar(stat = "identity") +
    geom_text(aes(x = 1, y = description, label = geneSet)) +
    ylab(NULL) +
    theme_bw(base_size = 15) +
    theme(
